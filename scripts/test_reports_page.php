...
$_GET=[];$_SERVER['REQUEST_METHOD']='GET';$_SERVER['REQUEST_URI']='/reports';$_SERVER['SCRIPT_NAME']='/index.php';$defaultResponse=(new ReportController())->index(Request::capture());$defaultReflection=new ReflectionClass($defaultResponse);$defaultContent=$defaultReflection->getProperty('content');$defaultContent->setAccessible(true);expectReport(strpos($defaultContent->getValue($defaultResponse),'Pilotage manager')!==false,'la vue Manager est affichée par défaut');

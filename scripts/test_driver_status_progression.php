...
try{
    ...
    $steps=[['accept','À préparer'],['prepare','Prête'],['load','Chargement'],['loaded','Chargée'],['start','En transit'],['arrive','Arrivée']];
    ...
    $history=(int)$pdo->query('SELECT COUNT(*) FROM delivery_status_history WHERE delivery_id='.(int)$deliveryId)->fetchColumn();expectDriverProgress($history===8,'la création et chaque transition, y compris Partie, sont journalisées');
    ...
    expectDriverProgress(DriverMission::nextAction('Affectée')['action']==='accept'&&DriverMission::nextAction('Chargement')['action']==='loaded','l’interface expose une seule prochaine action contextuelle');

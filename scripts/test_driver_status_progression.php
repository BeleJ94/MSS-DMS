...
try{
    ...
    $mission=DriverMission::findOwned($deliveryId);$html=View::render('driver-app/show',['title'=>$mission['reference'],'mission'=>$mission,'pod'=>DeliveryPod::summary($deliveryId),'pods'=>DeliveryPod::summaries($deliveryId),'driver'=>DriverMission::driver(),'activeMissionId'=>GpsTracking::activeMissionId(),'baseUrl'=>''],'layouts/driver-app');expectDriverProgress(strpos($html,'PROGRESSION DE LA MISSION')!==false&&strpos($html,'Commencer le chargement')!==false&&strpos($html,'Confirmer la préparation')===false,'la fiche chauffeur commence directement par le chargement');expectDriverProgress(strpos($html,'mission-hero stage-assigned')!==false,'la mission affectée reçoit son accent visuel sobre');
    $steps=[['load','Chargement'],['loaded','Chargée'],['start','En transit'],['arrive','Arrivée'],['unload','Déchargement']];
    ...
    $history=(int)$pdo->query('SELECT COUNT(*) FROM delivery_status_history WHERE delivery_id='.(int)$deliveryId)->fetchColumn();expectDriverProgress($history===9,'les étapes automatiques et chaque action chauffeur sont journalisées');
    ...
    expectDriverProgress(DriverMission::nextAction('Affectée')['action']==='load'&&DriverMission::nextAction('À préparer')['action']==='load'&&DriverMission::nextAction('Chargement')['label']==='Confirmer le chargement'&&DriverMission::nextAction('Chargée')['label']==='Confirmer le départ','le chauffeur ne voit que les actions chargement et départ avant la route');
    $change=Delivery::rollbackStatus($deliveryId,'Affectée','Correction du statut pendant le test');$corrected=$pdo->query('SELECT status FROM deliveries WHERE id='.(int)$deliveryId)->fetchColumn();$destinationStatus=$pdo->query('SELECT status FROM delivery_destinations WHERE delivery_id='.(int)$deliveryId.' ORDER BY stop_order LIMIT 1')->fetchColumn();expectDriverProgress($change['from']==='Déchargement'&&$corrected==='Affectée','un administrateur peut ramener une livraison au statut Affectée');expectDriverProgress($destinationStatus==='À livrer','le retour réinitialise la destination active de façon cohérente');
    $lastComment=$pdo->query('SELECT comment FROM delivery_status_history WHERE delivery_id='.(int)$deliveryId.' ORDER BY id DESC LIMIT 1')->fetchColumn();expectDriverProgress(strpos((string)$lastComment,'Correction administrateur :')===0,'la correction administrateur est journalisée avec son motif');

...
try{
    ...
    $mission=DriverMission::findOwned($deliveryId);$html=View::render('driver-app/show',['title'=>$mission['reference'],'mission'=>$mission,'pod'=>DeliveryPod::summary($deliveryId),'pods'=>DeliveryPod::summaries($deliveryId),'driver'=>DriverMission::driver(),'activeMissionId'=>GpsTracking::activeMissionId(),'baseUrl'=>''],'layouts/driver-app');expectDriverProgress(strpos($html,'PROGRESSION DE LA MISSION')!==false&&strpos($html,'Véhicule chargé')!==false&&strpos($html,'Confirmer la préparation')===false,'la fiche chauffeur propose une confirmation unique du chargement');expectDriverProgress(strpos($html,'mission-hero stage-assigned')!==false,'la mission affectée reçoit son accent visuel sobre');
    $steps=[['load','Chargée'],['start','En transit'],['arrive','Arrivée'],['unload','Déchargement']];
    ...
    expectDriverProgress(DriverMission::nextAction('Affectée')['label']==='Véhicule chargé'&&DriverMission::nextAction('Chargement')['action']==='load'&&DriverMission::nextAction('Chargée')['label']==='Confirmer le départ','une seule confirmation de chargement précède le départ');

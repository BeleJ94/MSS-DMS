...
try {
    ...
    $destination=$pdo->prepare('INSERT INTO delivery_destinations (delivery_id,stop_order,label,address_line,city,status,arrived_at) VALUES (:delivery,:position,:label,:address,"Lubumbashi","Arrivée",NOW())');
    ...
    $pdo->prepare('UPDATE deliveries SET status="Arrivée" WHERE id=:id')->execute(['id'=>$deliveryId]);$pdo->prepare('UPDATE delivery_destinations SET status="Arrivée",arrived_at=NOW() WHERE id=:id')->execute(['id'=>$destinationTwo]);

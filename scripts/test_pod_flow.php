...
function testDeliveryPhoto(): array { $image=imagecreatetruecolor(600,400);$white=imagecolorallocate($image,248,248,248);$blue=imagecolorallocate($image,22,58,103);imagefill($image,0,0,$white);imagefilledrectangle($image,100,100,500,300,$blue);ob_start();imagejpeg($image,null,88);$data=(string)ob_get_clean();imagedestroy($image);return ['mime'=>'image/jpeg','data'=>$data,'width'=>600,'height'=>400]; }
...
try {
    ...
    $photo=testDeliveryPhoto();
    DeliveryPod::createOwned($deliveryId,$destinationOne,['recipient_name'=>'Réceptionnaire Test 1','observations'=>'Premier arrêt reçu.','latitude'=>-11.6647,'longitude'=>27.4794,'accuracy'=>8.5],$photo);
    ...
    DeliveryPod::createOwned($deliveryId,$destinationTwo,['recipient_name'=>'Réceptionnaire Test 2','observations'=>'Dernier arrêt reçu.','latitude'=>-11.6648,'longitude'=>27.4795,'accuracy'=>7.5],$photo);
    $delivery=$pdo->query('SELECT status,delivered_at FROM deliveries WHERE id='.(int)$deliveryId)->fetch();
    ...
    expectPod(empty($pod['signature_data'])&&empty($pod['signature_mime']),'la livraison est enregistrée sans signature');
    ...
    expectPod(strpos($pdf,'SIGNATURE')===false&&strpos($pdf,'BON SIGN')===false,'le bon PDF ne contient aucune zone de signature');

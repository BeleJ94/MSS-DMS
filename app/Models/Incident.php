...
final class Incident
{
    ...
    {
        $sql='SELECT i.*,d.reference delivery_reference,d.status delivery_status,d.status_before_incident,c.company_name,dd.label site_name,dd.address_line site_address,dd.city site_city,CONCAT(dr.first_name," ",dr.last_name) driver_name,dr.phone driver_phone,v.registration_number,ru.name responsible_name,rbu.name reporter_name,resu.name resolver_name FROM driver_incidents i JOIN drivers dr ON dr.id=i.driver_id LEFT JOIN deliveries d ON d.id=i.delivery_id LEFT JOIN clients c ON c.id=d.client_id LEFT JOIN delivery_destinations dd ON dd.id=(SELECT dx.id FROM delivery_destinations dx WHERE dx.delivery_id=d.id AND dx.status NOT IN ("Livrée","Annulée") ORDER BY dx.stop_order LIMIT 1) LEFT JOIN vehicles v ON v.id=d.vehicle_id LEFT JOIN users ru ON ru.id=i.responsible_user_id LEFT JOIN users rbu ON rbu.id=i.reported_by LEFT JOIN users resu ON resu.id=i.resolved_by WHERE i.id=:id';

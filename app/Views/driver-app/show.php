...
<section class="mobile-card gps-audit is-collapsed" id="gpsAudit" data-mission-id="<?= (int)$mission['id'] ?>">
    <div class="gps-audit-head"><div><small>VÉRIFICATION GPS EN TEMPS RÉEL</small><h2>Historique des transmissions</h2></div><div class="gps-audit-actions"><button type="button" id="gpsAuditRefresh" aria-label="Actualiser"><i data-lucide="refresh-cw"></i></button><button type="button" id="gpsAuditToggle" aria-expanded="false"><span>Voir les détails</span><i data-lucide="chevron-down"></i></button></div></div>
    <div class="gps-audit-details" id="gpsAuditDetails">
...
<section class="mission-actions" data-mission-id="<?= (int) $mission['id'] ?>">
    ...
    <?php if ($mission['status'] === 'Déchargement' && $mission['unloading_complete']): ?><button class="primary success" type="button" data-pod-open><i data-lucide="badge-check"></i>Confirmer la livraison effectuée</button><?php endif; ?>
...
<div class="pod-modal" id="podModal" hidden aria-hidden="true">
    <section class="pod-sheet" role="dialog" aria-modal="true" aria-labelledby="podTitle">
        <header><button type="button" data-pod-close aria-label="Fermer"><i data-lucide="x"></i></button><span>BON DE LIVRAISON</span><h2 id="podTitle">Confirmer la livraison effectuée</h2><p><?= htmlspecialchars($mission['reference'].' · '.$mission['company_name'], ENT_QUOTES, 'UTF-8') ?></p></header>
        <form id="podForm" data-mission-id="<?= (int) $mission['id'] ?>" data-destination-id="<?= (int)$mission['current_destination_id'] ?>">
            ...
            <p class="pod-legal">La confirmation est horodatée et associée au chauffeur, à la destination, à la photo et à la position GPS.</p>
        </form>
    </section>
</div>
<?php endif; ?>

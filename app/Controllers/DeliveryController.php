...
final class DeliveryController extends Controller
{
    ...
    {
        if(!Auth::can('deliveries.view')){return $this->forbidden(false);}$delivery=Delivery::find((int)$request->param('id'));if(!$delivery){return new Response(View::render('errors/404',['title'=>'Livraison introuvable']),404);}$canViewRoute=Auth::can('tracking.history.view');$officeActions=array_values(array_filter(Delivery::allowedNext($delivery),function(string $status):bool{return in_array($status,['Annulée','Clôturée'],true);}));return $this->view('deliveries/show',$this->viewData(['title'=>$delivery['reference'],'page'=>'deliveries','delivery'=>$delivery,'allowedNext'=>$officeActions,'canManage'=>Auth::can('deliveries.manage'),'canDelete'=>Auth::hasRole('administrateur'),'canTransition'=>Auth::can('deliveries.status'),'canViewRoute'=>$canViewRoute,'usesLeaflet'=>$canViewRoute]));
    }

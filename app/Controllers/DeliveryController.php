...
final class DeliveryController extends Controller
{
    ...
    {
        $e=[];foreach(['client_id'=>'Sélectionnez un client.','client_site_id'=>'Sélectionnez un site de livraison.','scheduled_at'=>'La date prévue est obligatoire.'] as $field=>$message){if(trim((string)($d[$field]??''))===''){$e[$field]=$message;}}$scheduled=str_replace('T',' ',(string)($d['scheduled_at']??''));$date=\DateTime::createFromFormat('Y-m-d H:i',$scheduled);if(($d['scheduled_at']??'')!==''&&(!$date||$date->format('Y-m-d H:i')!==$scheduled)){$e['scheduled_at']='Date prévue invalide.';}if(!in_array($d['priority']??'Normale',['Basse','Normale','Haute','Urgente'],true)){$e['priority']='Priorité invalide.';}$goods=$d['goods']??[];if(!is_array($goods)||$goods===[]){$e['goods']='Ajoutez au moins une marchandise.';}else{foreach($goods as $row){$description=trim((string)($row['description']??''));$unit=trim((string)($row['unit']??''));$weight=$row['unit_weight_kg']??'';if(mb_strlen($description)<2||mb_strlen($description)>255||!is_numeric($row['quantity']??null)||(float)$row['quantity']<=0||$unit===''||mb_strlen($unit)>30||($weight!==''&&(!is_numeric($weight)||(float)$weight<=0))){$e['goods']='Chaque ligne doit avoir une désignation, une quantité positive et une unité valides.';break;}}}return $e;
    }
